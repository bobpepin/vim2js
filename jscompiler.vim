" This file is based on the code from https://github.com/vim-jp/vim-vimlparser
" Modified by Bob Pepin
...
function s:JavascriptCompiler.compile_function(node)
  ...
  if left =~ '^\(VimLParser\|ExprTokenizer\|ExprParser\|LvalueParser\|StringReader\|Compiler\|RegexpParser\)\.'
    ...
    if left[0:1] == '$s'
      call self.out('%s = function (%s) {', left, join(rlist, ', '))
    else
    ...
    call self.out('}')
    if left[0:2] != '$s.'
      call self.out('exports.%s = %s;', left, left)
    endif
  endif
  call self.emptyline()
endfunction
...
function! s:convert(in, out) abort
  try
    ...
    call writefile(["var $s = scriptVariables[module.id];"] + lines, a:out)
  catch
